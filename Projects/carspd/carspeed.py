...
# Aug 2020 - Running on Buster with opencv-contrib-dev, getting error "undefined symbol: __atomic_fetch_add_8"
# Use following to invoke:
# deskpi> LD_PRELOAD=/usr/lib/arm-linux-gnueabihf/libatomic.so.1 python3 carspeed.py

# CarSpeed Version 2.0 (modified by Alan McDonley)

# Mods:
# - fixed findContours() only returns two items now
# - added try/except KeyboardInterrupt for clean exit
# - put images in ./images folder
# - put csv files in ./csv folder
# - changed csv file ext from cvs to csv
# - creates csv and images folders if do not exist
# - added -c -csv optional argument to store csv list of events
# - added -d -display optional argument to display images
# - added -m -mouse optional arg to define area with mouse
# - added -t -track optional arg to show tracking detail
...
ap.add_argument("-c", "--csv", default=False, action='store_true', help="optional save data to csv file")
ap.add_argument("-d", "--display", default=False, action='store_true', help="optional output to display")
ap.add_argument("-m", "--mouse", default=False, action='store_true', help="optional use mouse to define watch area")
ap.add_argument("-t", "--tracking", default=False, action='store_true', help="optional show tracking detail")
args = vars(ap.parse_args())
print("Started with args:", args)
SAVE_CSV = args['csv']
SHOW_IMAGE = args['display']
MOUSE_AREA = args['mouse']
SHOW_TRACKING = args['tracking']

# must have window to define area with mouse
if MOUSE_AREA:
    SHOW_IMAGE = True
    SHOW_BOUNDS = True
    setup_complete = False
else:
    SHOW_BOUNDS = SHOW_IMAGE
    setup_complete = True

# define some constants
DISTANCE = 46  #<---- enter your distance-to-road value here
MIN_SPEED = 0  #<---- enter the minimum speed for saving images
# SAVE_CSV = False  #<---- record the results in .csv format in ./csv/carspeed_(date).csv

THRESHOLD = 15
MIN_AREA = 3000 # 175  # width * height of object in motion to trigger tracking
BLURSIZE = (15,15)
IMAGEWIDTH = 640
IMAGEHEIGHT = 480
...
# Upper Left = ix,iy 0=top left of image
# Lower Right = fx,fy
# custom  or   # center third high by center 80% width
ix = 63   # int(0.1 * float(IMAGEWIDTH))
iy = 191  # int(0.333 * float(IMAGEHEIGHT))
fx = 293  # int(0.9 * float(IMAGEWIDTH))
fy = 240  # int(0.666 * float(IMAGEHEIGHT))

# print("ix,iy: {},{} fx,fy: {},{}".format(ix,iy,fx,fy))
...
if SHOW_TRACKING and SHOW_IMAGE:
    # create a image diff window
    cv2.namedWindow("ImageDiff")
    cv2.moveWindow("ImageDiff", (25 + IMAGEWIDTH), 40)

    cv2.namedWindow("RefImage")
    cv2.moveWindow("RefImage", (25 + IMAGEWIDTH), (40+int(0.666 * IMAGEHEIGHT)))
...
if SAVE_CSV:
    # added csv/ path and changed file ext from cvs to csv - Alan
    csvfileout = "csv/carspeed_{}.csv".format(datetime.datetime.now().strftime("%Y%m%d_%H%M"))
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
  try:
    ...
    if SHOW_TRACKING and SHOW_IMAGE:
        cv2.imshow("ImageDiff", thresh)
        cv2.imshow("RefImage",base_image)
        cv2.waitKey(1)
    ...
    if motion_found:
        if state == WAITING:
            ...
            print(text_on_image)
            if SHOW_TRACKING: print("x-chg    Secs      MPH  x-pos width")
        else:
            ...
            if state == TRACKING:
                ...
                if SHOW_TRACKING: print("{0:4d}  {1:7.2f}  {2:7.0f}   {3:4d}  {4:4d}".format(abs_chg,secs,mph,x,w))

                # show the frame and check for a keypress
                if SHOW_IMAGE:
                    mph_prompt = "%.0f mph" % mph
                    prompt_on_image(mph_prompt)
                    cv2.imshow("Speed Camera", image)
                ...
                if ((x <= 2) and (direction == RIGHT_TO_LEFT)) \
                    ...
                    if (last_mph > MIN_SPEED):    # save the image
                        ...
                        # Save the image to disk in ./images folder - Alan

                        # use the following image file name if you want to be able to sort the images by date
                        # imageFilename = "images/car_at_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + ".jpg"

                        # use the following image file name if you want to be able to sort the images by speed
                        imageFilename = "images/car_at_%02.0f" % last_mph + "_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + ".jpg"
